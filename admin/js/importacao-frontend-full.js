...
function inicializaOverlay( ) {
    ...
    if ( overlay ) {
        ...
        const messageComponent = document.getElementById( 'importacao-info' );
        if ( messageComponent ) {
...
function iniciarApagarImoveis( ) {
    if ( PineduAjax?.environment === 'development' ) console.log( 'prepararImportarImoveis' );
    const before = function ( ) {
        alteraInfo( 'Excluir todos os Imóveis...' );
        alteraMessage( 'Preparando exclusão de Imóveis.' );
        alteraProgresso( 0 );
    }, success = function ( data ) {
        const info = 'Excluir todos os Imóveis...';
        const progresso = 0;
        const message = 'Preparação exclusão de Imóveis realizada com sucesso!';
        if ( data.success === true ) {
            alteraInfo( info );
            alteraMessage( message );
            alteraProgresso( progresso );
...
function selecionarApagarImoveis( dadosLogin, progresso ) {
    ...
    const before = function ( ) {
        ...
        if ( data.success === true ) {
            ...
            alteraMessage( message );
            alteraProgresso( 100 );
            excluirImoveisFrontEnd( data.imoveis, data.total, 0 );
        } else {
            finalizarImportacaoFrontEnd( 0, 'Finalizando Exclusão.', 'Exclusão de Imóveis realizada com sucesso!' );
            exibeFechar( );
        }
    }, error = errorDoPost;
    const args = {};
...
function excluirImoveisFrontEnd( listaIds, total, progresso, callBackFunction ) {
    ...
    const before = function ( ) {
        alteraInfo( 'Exclusão de Imóveis...' );
        alteraMessage( `Excluindo Imóvel: ${listaIds[progresso].referencia}` );
        const avanco = ( ( progresso / total ) * 100 );
        ...
        if ( data.success === true ) {
            ...
            if ( (progresso + 1) < total ) {
                ...
                if ( typeof callBackFunction == 'function' ) {
                    ...
                    finalizarImportacaoFrontEnd( total, 'Finalizando Exclusão.', 'Exclusão de Imóveis realizada com sucesso!' );
                    exibeFechar( );
                }
            }
        } else {
            ...
            alteraMessage( data.message );
            alteraProgresso( 0 );
            finalizarImportacaoFrontEnd( total, 'Finalizando Exclusão.', 'Exclusão de Imóveis terminada de forma anormal!' );
        }
    }, error = errorDoPost;
    const args = {
        id: JSON.stringify( listaIds[progresso].id )
    };
