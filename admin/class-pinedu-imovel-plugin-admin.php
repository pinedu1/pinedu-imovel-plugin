...
class Pinedu_Imovel_Plugin_Admin {
    ...
    public function enqueue_scripts( ) {
        ...
        if ( ! isset($options['imoveis_importar_lote'])) {
            $options['imoveis_importar_lote'] = 10;
    ...
    public function define_settings( ) {
        ...
        // Adicionar seções de configuração (Integração)
        ...
        // Adicionar seções de configuração Email
        ...
        // NOVA SEÇÃO: Comportamento SEO
        add_settings_section( 'secao_seo', 'Comportamento SEO', [$this, 'exibir_secao_seo'], 'pinedu-imovel' );
        add_settings_field( 'desativar_yoast', 'Desativar Yoast SEO', [$this, 'exibir_desativar_yoast'], 'pinedu-imovel', 'secao_seo' );
        add_settings_field( 'desativar_rankmath', 'Desativar Rank Math', [$this, 'exibir_desativar_rankmath'], 'pinedu-imovel', 'secao_seo' );
        add_settings_field( 'desativar_aioseo', 'Desativar All in One SEO', [$this, 'exibir_desativar_aioseo'], 'pinedu-imovel', 'secao_seo' );
    }
    ...
    // Callbacks de Exibição dos Campos de SEO
    public function exibir_secao_seo() {
        echo '<p>Configure quais plugins de SEO o sistema deve sobrepor para injetar as tags (Meta Description e Open Graph) dinâmicas exclusivas deste plugin.</p>';
    }

    public function exibir_desativar_yoast() {
        $options = get_option( 'pinedu_imovel_options', [] );
        $checked = isset( $options['desativar_yoast'] ) ? checked( $options['desativar_yoast'], 'on', false ) : '';
        echo '<input type="checkbox" name="pinedu_imovel_options[desativar_yoast]" ' . $checked . ' />';
        echo '<p class="description">Desliga a injeção nativa de Meta Description, Facebook e Twitter do <b>Yoast SEO</b>.</p>';
    }

    public function exibir_desativar_rankmath() {
        $options = get_option( 'pinedu_imovel_options', [] );
        $checked = isset( $options['desativar_rankmath'] ) ? checked( $options['desativar_rankmath'], 'on', false ) : '';
        echo '<input type="checkbox" name="pinedu_imovel_options[desativar_rankmath]" ' . $checked . ' />';
        echo '<p class="description">Desliga a injeção nativa de Meta Description, Facebook e Twitter do <b>Rank Math</b>.</p>';
    }

    public function exibir_desativar_aioseo() {
        $options = get_option( 'pinedu_imovel_options', [] );
        $checked = isset( $options['desativar_aioseo'] ) ? checked( $options['desativar_aioseo'], 'on', false ) : '';
        echo '<input type="checkbox" name="pinedu_imovel_options[desativar_aioseo]" ' . $checked . ' />';
        echo '<p class="description">Desliga a injeção nativa de Título e Descrição do <b>All in One SEO (AIOSEO)</b>.</p>';
    }
    ...
    public function exibir_importacao_andamento( ) {
        ...
        if ( isset( $options['importacao_andamento'] ) ) {
            $importacao_andamento = ($options['importacao_andamento'] == true || $options['importacao_andamento'] == 'on');
        }
    ...
    public function exibir_tipo_seguranca( ) {
        $options = get_option( 'pinedu_imovel_options', [] );
        $tipo_seguranca = $options['tipo_seguranca'] ?? '';
